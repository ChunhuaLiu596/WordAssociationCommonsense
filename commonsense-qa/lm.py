...
def main():
    ...
    parser.add_argument('--mode', default='train', choices=['train', 'extract', 'eval', 'pred', 'decode'], help='run training or evaluation')
    parser.add_argument('--save_dir', default=f'./saved_models/{args.dataset}.{args.encoder}.lm/{args.run_folder}', help='model output directory')
    parser.add_argument('--train_with_decode', type=bool_flag, default=False, help='decode after training')
...
def train(args):
    ...
    num_params = num_params_encoder + num_params_decoder
    ...
    try:
        for epoch in range(int(args.n_epochs)):
            if args.encoder_type=="PLM" and epoch == args.unfreeze_epoch:
                unfreeze_net(model.encoder)
            if args.encoder_type=="PLM" and epoch == args.refreeze_epoch:
                freeze_net(model.encoder)
            model.train()
            ...
            # print('| epoch {:5} | train_acc:{:7.4f} | dev_acc {:7.4f} | test_acc {:7.4f} |'.format(epoch, train_acc, dev_acc, test_acc))
...
def decode(args):
    ...
    for eval_set, filename in zip([dataset.dev(), dataset.test()], ['decode_dev.txt', 'decode_test.txt']):
        ...
        with torch.no_grad():
            for qids, labels, *input_data in tqdm(eval_set):
                ...
                for i, (qid, label, pred) in enumerate(zip(qids, labels, predictions)):
                    if label == pred:
                        correct.append('*' * 60)
                        correct.append('id: {}'.format(qid))
                        correct.append('question: {}'.format(statement_dic[qid]['question']))
                        correct.append('answer: {}'.format(statement_dic[qid]['answers'][label.item()]))
                        correct.append('prediction: {}'.format(statement_dic[qid]['answers'][pred.item()]))
                        for j, answer in enumerate(statement_dic[qid]['answers']):
                            correct.append('{:25} '.format('[{}. {}]{}{}'.format(chr(ord('A') + j),
                                                                                   answer,
                                                                                   '*' if j == label else '',
                                                                                   '^' if j == pred else '')
                                                            ))
                        ...
                        wrong.append('*' * 60)
                        wrong.append('id: {}'.format(qid))
                        wrong.append('question: {}'.format(statement_dic[qid]['question']))
                        wrong.append('answer: {}'.format(statement_dic[qid]['answers'][label.item()]))
                        wrong.append('prediction: {}'.format(statement_dic[qid]['answers'][pred.item()]))
                        for j, answer in enumerate(statement_dic[qid]['answers']):
                            wrong.append('{:25} '.format('[{}. {}]{}{}'.format(chr(ord('A') + j),
                                                                                   answer,
                                                                                   '*' if j == label else '',
                                                                                   '^' if j == pred else '')
                                                            ))
        ...
        outputs.insert(0, "accuracy:{}".format(accuracy))
        outputs.insert(1, "correct_num: {}".format(n_correct))
        outputs.extend(correct)
        outputs.extend(["wrong_num: {}".format(len(wrong))])
        outputs.extend(wrong)
        output_path = os.path.join(args.save_dir, filename)
        with open(output_path, 'w') as fout:
            for line in outputs:
                fout.write(line + '\n')
        print(f'outputs saved to {output_path}, accuracy:{accuracy}')
    print("***** done *****")
    print()
